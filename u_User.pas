...
// Procedure to handle the "Display" button click event for various display options.
...
begin
  try
    with dmco2 do
    begin
      ADOQueryfootprint.filtered := false;
      ADOQueryfootprint.filter := 'EmissionDate >= ' + quotedstr
        ...
        + ' and EmissionDate < ' + quotedstr(FormatDateTime('yyyy/mm/dd',
          DateTimeTo.Date));
      ADOQueryfootprint.filtered := true;
    end;
  except
...
// Procedure to handle the "Statistics" button click event for displaying statistics.
procedure Tfrm_Users.btnStatisticsClick(Sender: TObject);
begin
  try
    if RadioStatistics.itemindex = 0 then // Allow to see which type of user the information is being submitted
    begin
